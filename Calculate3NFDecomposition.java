...
public class Calculate3NFDecomposition extends CalculateDecomposition {
	...
	public void decompose(final boolean force3NFDecomposition) {
		...
		int counter = 0;
		for (Functional_Dependency fd : getInputRelation().getMinimalCover()) {
			...
			List<Functional_Dependency> decomposedFD = RDTUtils.fetchFDsOfDecomposedR(RDTUtils.getSingleAttributeMinimalCoverList(getInputRelation().getMinimalCover(), getInputRelation()),
					decomposedAttrs);
		...
		if (originalAttributes.size() > addedAttributes.size()) {
			...
			appendOutputMsg(". ");
			List<Functional_Dependency> emptyFD = RDTUtils.fetchFDsOfDecomposedR(RDTUtils.getSingleAttributeMinimalCoverList(getInputRelation().getMinimalCover(), getInputRelation()),
					missingAttributes);
		...
		if (foundKey != null) {
			...
			List<Functional_Dependency> emptyFD = RDTUtils.fetchFDsOfDecomposedR(RDTUtils.getSingleAttributeMinimalCoverList(getInputRelation().getMinimalCover(), getInputRelation()), addedKeyAttrs);
